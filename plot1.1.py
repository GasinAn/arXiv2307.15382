...
sample = np.sort((1-(np.random.choice(d_l, size=10**8)
                    /np.random.normal(loc=42.9, scale=3.2, size=10**8))**2)
                 /np.random.normal(loc=42.9, scale=3.2, size=10**8))
...
axs[0].set_xlabel('$\\overline{(dG/dD_L)}/G$')
axs[0].set_ylabel('$p(\\overline{(dG/dD_L)}/G)$')
axs[0].set_xlim((-0.035,+0.035))
...
for i in range(len(x)):
    ...
    if (s >= 0.0025):
        x_p = x[-2-i]
        break
print("%f" % ((1-1.8/2.5)/2.5e3))
print("%f" % ((1-(1.8-0.1)/2.5)/2.5e3))
print("%f" % ((1-(1.8+1.1)/2.5)/2.5e3))
axs[1].plot(x, y, c='tab:orange')
axs[1].axvline(x_m, c='tab:grey', ls='--')
axs[1].axvline(x_p, c='tab:grey', ls='--')
axs[1].set_title('GW190521')
axs[1].set_xlabel('$\\overline{(dG/dD_L)}/G$')
axs[1].set_ylabel('$p(\\overline{(dG/dD_L)}/G)$')
